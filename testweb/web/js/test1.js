let currentFileIndex = 0; // Để theo dõi tệp nào đang được đọc
const files = ["./js/datatest.json","./js/datatest1.json","./js/data.json","./js/datatest2.json"]; // Đường dẫn đến các tệp JSON

function updateGraph(filePath) {
...
updateGraph(files[currentFileIndex]);

// lap di lap lai
setInterval(function() {
    currentFileIndex = (currentFileIndex + 1) % files.length; // chia du de vong lai neu khong se bi qua so luong phan tu
    updateGraph(files[currentFileIndex]);
}, 4000);
